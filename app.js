...
onAuthStateChanged(auth, (user) => {
    if (user) {
      if(user.emailVerified === true){
        location.replace("signup/Forget Password/Email Verification/Chats/chat.html")
      }
...
async function loginUser() {
    ...
    if (checkEmail !== "" || checkPassword !== "") {
        try {
            ...
            location = `signup/Forget Password/Email Verification/Chats/chat.html`
        }
        catch (error) {
...
function clearErrorAfter3Sec() {
    stopSetTime = setTimeout(clearError, 4000)
}